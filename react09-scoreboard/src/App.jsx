...
function App() {
  ...
  const addPlayerProcess = (pName) => {
    ...
    let addPlayer = {idx: setNextVal, name: pName, score: 0}; 
    ...
    //새로운 플레이어 추가를 위해 시퀸스 증가
    setNextVal(nextVal+1);
  }
  
  //점수의 증감을 위한 함수. 매개변수는 증감을 위한 플래그, 선수의 일련번호.
  const scoreChangProcess = (flag, playerIdx) => {
  ...
  const deletePlayerProcess = (playerIdx) => {
    ...
    //수정을 위한 함수
    const editplayersProcess = (idx, name) => {
