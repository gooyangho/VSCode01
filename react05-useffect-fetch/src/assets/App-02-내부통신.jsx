...
const GlobalTop = (props) => {
  ...
  차우 useEffect가 실행되면 json파일의 내용을 읽어와서 값을 변경한다. */
  var [myList, setMyList] = useState([]);

  //이 컴포넌트가 1차 렌더링된 후 실행되는 훅 정리
  useEffect(function(){
  ...
  for(var i=0 ; i<myList.length; i++) {
    ...
    listTag.push(
      <li key={data.id}>
        ...
        dataset.id를 통해 읽어올수 있다. 이 속성에 게시물의 일련번호인
        num을 설정하고 있다. */}
        <a href={data.id} data-id={data.num} onClick={(e)=>{
          e.preventDefault();
          //여기서 게시물의 일련번호를 부모컴포넌트로 전달된다.
